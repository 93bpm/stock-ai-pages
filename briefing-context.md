...
**Stooq 캐럿 prefix 티커 실패 시 fallback** (실제 검증으로 발견된 한계):

| 실패 티커 | 대체 방법 |
|---|---|
| `^vix` | WebSearch로 `"VIX close YYYY-MM-DD"` 검색해 값 추출 |
| `^sox` | `soxx.us` (iShares 반도체 ETF) — 지수와 약간 다르지만 방향성 동일 |
| `^dxy` | `uup.us` (Invesco DB Dollar Bullish ETF) 또는 WebSearch로 "DXY close" |
| `10us.b` | WebSearch로 `"US 10-year Treasury yield YYYY-MM-DD"` |
| `^kosdaq` | WebSearch로 한경/이데일리 마감 기사에서 코스닥 종가 추출 |
| `xlre.us` (또는 다른 ETF) 빈 응답 | **finviz** `https://finviz.com/quote.ashx?t=XLRE` — 일간 종가·등락률 정확 추출 가능 |

→ Stooq 실패 시 그 항목을 비우지 말고 위 fallback 거치고, 그래도 안 되면 §6의 마스킹 형식 적용.
...
2. dates 배열에 새 날짜 추가 (이미 있으면 skip — 중복 방지)
3. dates 배열 오름차순 정렬
4. latest = dates의 마지막 원소
5. updatedAt = 현재 KST ISO 시각
6. briefing/manifest.json 저장
```
