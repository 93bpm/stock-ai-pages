...
| 버전 | 날짜 | 주요 변경 |
|---|---|---|
| **v1.6.1** | 2026-06-09 | **schema 과엄격 완화 + 6/8 백필** — ① `kr.flow` `minItems` 3→2 (주말·휴장으로 야간선물/ADR을 못 구하면 2개 허용 — 실제 routine 출력과 일치, `data-quality.md §201`이 이미 "2개 이하 가능"이라 명시했던 schema↔문서 모순 해소). ② `WatchItem.badgeType` enum에 `down` 추가(`["up","down","default"]`) — routine이 하락 종목에 자연스럽게 쓰던 값. 화면(`index.html:783`)은 이미 `up`만 빨강·나머지 파랑으로 처리해 한국 관습(상승=빨강·하락=파랑)과 일치하므로 **코드 무수정**. `outputs.md §4.5` badgeType 매핑표·`operations.md §3` flow 카운트(3→2~3) 동기화. → 6/6~6/9 전 데이터 검증 **HARD 0건화**, `VALIDATE_STRICT=1` 게이트 준비 완료. ③ **6/8 백필**: claude.ai 예약 실행 missed tick(콘솔 이력·커밋·usage 모두 0)으로 누락된 6/8 브리핑을 routine 충실 재현(6/5 '검은 금요일' 회고 + 재개장 첫날 전망)으로 복원 |
