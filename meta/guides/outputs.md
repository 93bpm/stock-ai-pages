...
| `model` | string | `"claude-fable-5"` (routine prompt 명시값) | **자체 추측 금지** — routine prompt(Step 8)에 명시된 모델 ID를 그대로 기록. 명시 안 되면 `"unknown"`. LLM 자체 보고는 부정확 (5/26~5/28 사례). 모델 교체 시 prompt Step 8 값만 수정 — v1.6.1 |
